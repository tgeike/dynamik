...
# ╔═╡ f4de3b56-4f01-4c49-bbfe-92d84b5cbe23
using Plots,Unitful,DifferentialEquations

# ╔═╡ 58f8e8e7-205f-43f7-8278-5cc0d0c20fe4
using Interpolations

# ╔═╡ e07cf4a8-944e-11ee-0398-7b530d8b5455
md"""
# Kinetik mit Julia
## Seilzugantrieb
Wir untersuchen einen Antrieb mit einem Seilzug, wie er beispielsweise im Roboter [Myon](http://www.neurorobotik.de/robots/myon_de.php) verwendet wird. Die konkreten Zahlenwerte beziehen sich nicht auf den Roboter Myon.
![Myon](http://www.neurorobotik.de/img/robots/myon/IMG_4060.jpg) (Bild von der verlinkten Website)"""

# ╔═╡ d45c6842-8ab7-4edb-83ca-878ac23d83ed
md"""
### Vorbereitung: Pakete laden, Zahlenwerte festlegen
Wir benutzen das Paket `Unitful` für eine Berechnung mit Einheiten. Das Paket `DifferentialEquations` wird für die numerische Lösung der Bewegungsgleichung (Anfangswertproblem = AWP) benötigt."""

# ╔═╡ 360ec5f8-79c7-425d-908f-ba93fdf212f2
md"""Im ersten Schritt werden die Größen mit den Einheiten definiert. Beachten Sie: die Größen können in beliebigen zulässigen Einheiten notiert werden. Für die Länge ``l`` kann als Einheit beispielsweise Zentimeter oder Millimeter, aber auch Inch (Zoll) verwendet werden. Das ist insbesondere dann praktisch, wenn Entwickler:innen in internationalen Teams zusammenarbeiten und Zahlenwerte aus verschiedenen Quellen stammen. *Unterschätzen Sie nicht den Wert, den die Berücksichtigung der Einheiten in einer Programmiersprache für technische Berechnungen hat.*"""

# ╔═╡ b6a75595-4a19-4643-aafd-f3f50e0edc7f
begin
	m2 = 10.0u"kg"
	l = 40.0u"cm"
	JS2 = 0.9u"kg*m^2"
...
# ╔═╡ fee4a9b3-88db-4283-880c-ea0c9b5793d4
md"""Um den Zahlenwert für `JS2` einschätzen zu können, wird zum Vergleich das MTM einer punktförmigen Masse `m2` im Abstand `l` vom Schwerpunkt S berechnet."""

# ╔═╡ 5f8ed062-2388-4c5b-a366-d5f0abbc71e1
uconvert(u"kg*m^2",m2*l^2)

# ╔═╡ a8d9fee3-4a7f-4a09-b1f3-39ba888a36e9
md"""
### Berechnung der Normalspannung im Seil
Berechne die Seilkraft bei gegebenem Antriebsmoment ``M_1`` und die daraus resultierende Normalspannung ``σ``."""

# ╔═╡ 906d4aa8-f568-46db-ac14-4339953467a7
F1 = uconvert(u"N", M1/r1)

# ╔═╡ 86b986a5-f31a-4358-a7a7-2427a475eeec
σ = F1/(0.25*π*dS^2)

# ╔═╡ e7ae1871-87f0-4cac-b535-21c3dfe1a70e
md"""Die berechnete Normalspannung ``σ`` muss mit der zulässigen Normalspannung (z. B. 80% der Streckgrenze) verglichen werden. Wenn die berechnete Spannung größer als die zulässige Spannung ist, müssen Seile mit größerem Durchmesser eingebaut werden."""

# ╔═╡ dffa68fa-43a3-4a9b-86da-ddac26598cd8
md"""
### Motorsteuerung und Festlegung des Parametersatzes für die Bewegungsgleichung
"""

# ╔═╡ 36a2e04f-3c9a-46a3-b742-4e6606492e26
md"""Für das Drehmoment des Motors soll prinzipiell eine Abhängigkeit von den Zustandsgrößen ``\underline{z}`` und der Zeit ``t`` möglich sein. Diese Abhängigkeit steckt in der Funktion ``\chi`` (hier `chi`). Im einfachsten Fall liefert diese Funktion unabhängig vom Zustand und der Zeit stets 1.0 zurück."""

# ╔═╡ 1c3ccb99-bbb3-4824-845b-09ad6f84da44
chi(z,t) = 1.0

# ╔═╡ e54ff874-7846-4b65-9dd7-a9cffc2b5eb0
md"""In der Bewegungsgleichung sind zwei Konstanten der Dimension 1/(Zeit²) zu berücksichtigen. Diese werden hier berechnet und in die Einheit 1/s² umgerechnet.
Hinweis: Das Erzwingen der Umrechnung auf die Einheit 1/s² mittels `unconvert` ist nicht notwendig für die weitere Berechnung. Aber es liest sich für die Anwender:innen deutlich leichter!

Anschließend werden die beiden Größen in einer `struct` (hier `Systemparameter` genannt) gespeichert."""

# ╔═╡ bc63987a-fb3a-48d9-9e79-d07aaee09791
c1 = uconvert(u"s^-2", (M1/JB2)*(r2/r1))

# ╔═╡ 3ca49aaa-3cd9-453a-89a0-4078374fc2f3
c2 = uconvert(u"s^-2", m2*l*g/JB2)

# ╔═╡ 5bc8aefa-03ee-4548-a9d3-b93c3187c859
struct Systemparameter 
	c1::Float64
	c2::Float64
end

# ╔═╡ eb16abd0-e188-47c7-bdcb-701a1c235c36
md"""Das numerische Lösen des AWP soll ohne Einheiten erfolgen. Selbst wenn die Implementierung des Dgl-Lösers die Verwendung einheitenbehafteter Größen zulässt, scheint es dem Autor sicherer zu sein, mit *reinen* Zahlenwerten zu arbeiten. 

Wie ist vorzugehen? Die beiden Systemparameter ``c_1`` und ``c_2`` sind in vorab definierte Einheiten (hier 1/s²) umzurechnen und dann sind die Einheiten zu entfernen, so dass die Zahlenwerte übrig bleiben. Das erledigt der Befehl `ustrip`, der die Umrechnung und anschließende Entfernung der Einheiten für uns übernimmt. 
Wenn man so vorgeht, dann bedeutet dies, dass die Zeit in Sekunden zu messen ist. Die Winkel sollen in rad gemessen werden. Konkret: Wenn der Dgl-Löser Winkelgeschwindigkeiten liefert, sind diese in der Einheit rad/s bzw. 1/s.

*****

Der Vorteil dieses *hybriden* Vorgehens liegen auf der Hand. Anwender:innen können im oberen Teil dieses Dokuments die vorliegenden Größen in beliebigen Einheiten eingeben. Problemlos kann bei einer Änderung von Zahlenwerten auch die Einheit anders gewählt werden. Ich kann heute mit 30 mm für den Radius rechnen und für eine andere Konstruktion morgen den Wert auf 2 inch setzen. Im weiteren Verlauf kommt es ausschließlich auf die beiden Konstanten ``c_1`` und ``c_2`` an, die völlig unabhängig von den Einheiten der Eingangsgrößen in die Einheit 1/s² umgerechnet werden.
"""

# ╔═╡ d8833cdb-6e45-4ae9-877d-011b7a05794b
param = Systemparameter(ustrip(Float64, u"s^(-2)", c1), ustrip(Float64, u"s^(-2)", c2))

# ╔═╡ 103d6d4d-d633-4441-a865-4460d27efb93
md"""
### Bewegungsgleichung definieren und numerisch lösen
Im nächsten Schritt wird die Bewegungsgleichung notiert und numerisch gelöst. Die Dgl. ist in der Größe ``\varphi_2`` formuliert. D. h. wir berechnen Drehwinkel ``\varphi_2`` und die Winkelgeschwindigkeit ``\dot\varphi_2`` der durch B gehenden Welle.
Die Bewegungsgleichung bedeutet, dass der Arm beliebig um die Achse in B gedreht werden kann. In der Praxis wäre es auch denkbar, dass es bei einem gegebenen Winkel einen Anschlag gibt, über den eine weitere Drehung nicht möglich ist. Zudem könnten Reibungseffekte ebenfalls berücksichtigt werden.

> Denken Sie daran, dass die Bewegungsgleichung eine Dgl. 2. Ordnung ist und daher in ein System 1. Ordnung umgeschrieben werden muss!
"""

# ╔═╡ ddceb768-8dd0-4cf2-a39f-b4ee089d5fcd
function bewegdgl!(F,z,p,t)
	# z = [Winkel, Winkelgeschwindigkeit] 
	F[1] = z[2] 
	F[2] = p.c1*chi(z,t) - p.c2*cos(z[1])
end

# ╔═╡ 33ad7f90-66e4-44cc-934f-251fb0120d3b
md"""Das Lösen des AWP erfolgt über den Befehl `solve`, wobei das zu lösende AWP vorab über `ODEProblem` zu definieren ist.

Wenn die Ausgabe nicht über ein Semicolon unterdrückt wird, erscheinen die ersten Zeilen der Lösungstabelle. Die Spalte `timespan` gibt die Zeitpunkte aus, zu denen die Lösung tabelliert ist. Die erste Spalte der Lösung (`value1`) enthält den Winkel in rad. Die zweite Spalte (`value2`) enthält die Winkelgeschwindigkeit in rad/s, wobei alle drei Spalten nur Zahlenwerte enthalten. Die Einheiten müssen wir später wieder hinzufügen.

> Im vorliegenden Fall ist eine Zeit `tende` gegeben, zu der der Löser die Berechnung beenden soll. Zusätzlich ist eine Abbruchbedingung `callback =` definiert. Die Berechnung soll beendet werden, wenn der Arm in einer senkrechten Position angelangt ist.
"""

# ╔═╡ d6a80102-d743-4c80-a5f9-f00dcce07bb2
begin 
	tende = 5.0
	tspan = (0.0,tende)
	z_start = [0.0; 0.0]
	prob1 = ODEProblem(bewegdgl!,z_start,tspan,param)
	condition(u, t, integrator) = abs(u[1]) - 0.5*π
	affect!(integrator) = terminate!(integrator)
	cb = ContinuousCallback(condition, affect!)
    sol1 = solve(prob1, Tsit5(), callback = cb, 
		reltol=1e-8, abstol=1e-8, saveat=0.01)
end

# ╔═╡ 05ff1b44-d43b-4cd8-937e-e19ca34e5d47
md"""
### Post Processing
#### Winkel und Winkelgeschwindigkeit der Welle in B
Im ersten Schritt werden Drehwinkel und Winkelgeschwindigkeit der durch B gehenden Welle geplottet."""

# ╔═╡ 74637fb4-74c7-4c3a-8ebc-b51eda2edfe5
plot(sol1,linewidth=2,xaxis="t",title=["Drehwinkel (rad)" "Winkelgeschw.
(rad/s)"],layout=(1,2), label = false)

# ╔═╡ a6ef72d4-f0e6-49fd-86e7-20422c6bb1d1
sol1.t[end]

# ╔═╡ 5a611cb3-a409-424f-9cca-3d315921a346
md"""Der Winkel 0 rad bedeutet die horizontale Lage des Arms. Der Winkel π/2 rad entspricht der vertikalen Lage (Arm senkrecht nach oben).

Experimentieren Sie, welche Ergebnisse sich ergeben, wenn das Motordrehmoment verringert oder vergrößert wird. Konkret: was passiert, wenn `M1` so verkleinert wird, dass `c1` etwas kleiner als `c2` ist? Wie wirkt sich eine Abhängigkeit des Motordrehmoments beispielsweise vom Winkel aus? Hinweis: Funktion `chi` anpassen."""

# ╔═╡ 01f6775b-9cd9-4e7e-a627-4503b171444a
md"""
#### Kinetische Energie und Leistung, Bilanz der kinetischen Energie prüfen
Im nächsten Schritt sollen die kinetische Energie und die externe Leistung untersucht werden.

Konkret wollen wir die Änderung ``\dot E_\mathrm{kin}`` der kinetischen Energie mit der Leistung ``P_\mathrm{e}`` der eingeprägten Kräfte und Kraftmomente vergleichen."""

# ╔═╡ 0d74b3d8-172a-4404-b435-b7e172fc3765
md"""Zur Erinnerung: Die erste Spalte der Lösung (`value1`) enthält den Winkel ``\varphi_2`` in rad. Die zweite Spalte (`value2`) enthält die Winkelgeschwindigkeit ``\omega_2 = \dot\varphi_2`` in rad/s. Um mit einheitenbehafteten Größen weiterzurechnen, fügen wir die Einheit 1/s zu den Zahlenwerten der Winkelgeschwindigkeit hinzu. Zudem erzeugen wir eine Spalte mit den Zeitpunkten, der wir die Einheit s hinzufügen und eine Spalte für die Winkel ``\varphi_2``."""

# ╔═╡ 31cc19fd-d03c-406d-90b6-f78f655cf6b8
ω2_tab = sol1[2,:]u"s^-1";

# ╔═╡ f118b115-6d79-4488-b509-3c77f6b3feeb
t_tab = sol1.t*1u"s";

# ╔═╡ 775da64a-fcd0-44e1-9d62-5342c171b3b8
length(t_tab)

# ╔═╡ bb617e4c-815d-4d08-b21b-452fd5d09220
ϕ2_tab = sol1[1,:];

# ╔═╡ 5c028234-76b1-414f-8a94-ffe78eb4212a
md"""Nun berechnen wir die kinetische Energie des Systems. Da wir die Massenträgheit auf der Antriebswelle 1 vernachlässigen und B als raumfest angenommen wird, lässt sich die gesamte kinetische Energie durch einen rotatorischen Energieterm für den Arm/Körper 2 (mit Bezug auf den körperfesten Fixpunkt B) schreiben. Wir erzwingen die Umrechnung in die Einheit Joule."""

# ╔═╡ b497cce8-03d6-4bfb-bd2e-2d470f0ee63e
Ekin_tab = uconvert.(u"J", 0.5*JB2*ω2_tab.^2);

# ╔═╡ 02aaaa0d-9cdb-4a44-8648-2c0d0f0e04e1
md"""
Um die zeitliche Änderung der kinetischen Energie an gleich vielen Datenpunkten (Zeitpunkten) zu erhalten, wie alle anderen Daten, müssen wir etwas mehr rechnen, als man auf den ersten Blick erwartet. Mit dem `diff`-Befehl wird eine Spaltenmatrix erzeugt, die die Differenz zwischen den benachbarten Elementen einer gegebenen Spaltenmatrix enthält. Wenig überraschend hat die auf diesem Wege erzeugte Spaltenmatrix einen Eintrag weniger als die ursprüngliche Matrix.

**Auftrag:** Erläutern Sie, was die folgenden drei Codezeilen machen."""

# ╔═╡ ac32e580-4ee7-43de-ad3e-0bb1d220e067
aux1 = uconvert.(u"W", diff(Ekin_tab)./diff(t_tab));

# ╔═╡ 2abbf862-f8d2-4749-8963-4b38f524231a
aux2 = push!(0.5*(aux1[1:end-1] + aux1[2:end]), aux1[end]);

# ╔═╡ 144f914d-acf1-41a4-959a-d912716d1990
dEkin_tab = append!([aux1[1]],aux2);

# ╔═╡ ba1930cb-9427-4b3d-a1a3-dcbeeba0a67b
md"""Nun stellen wir die kinetische Energie und die zeitliche Änderung der kinetischen Energie in Abhängigkeit von der Zeit dar. Hinweis: Beim Plotten einheitenbehafteter Größen wird die Einheit automatisch den Achsenbeschriftungen hinzugefügt.
"""

# ╔═╡ e19241fc-fdbf-4871-8b5f-29a595db6783
begin
    p1 = plot((t_tab, Ekin_tab), label = false, xlabel = "t", ylabel = "Ekin", w = 2)
	p2 = plot((t_tab, dEkin_tab), label = false, xlabel = "t", ylabel = "dEkin/dt", w = 2, linestyle = :solid, color = :red)
	pl1 = @layout [a b]
	plot(p1,p2,layout = pl1,size=(600,300),fontfamily="segoe")
end

# ╔═╡ d4ba8670-40a2-4dee-adc9-95f11b482604
md"""
Nun berechnen wir die Leistung der eingeprägten Kräfte und Kraftmomente. Konkret sind dies im vorliegenden Fall die Leistung des Motordrehmomentes ``M_1`` und die Leistung der Gewichtskraft ``m_2 g``.

Vorsicht: Das funktioniert vorerst nur für ``\chi= 1``."""

# ╔═╡ 4cf769e8-3e3e-4a69-a1a0-6ff35c61d236
Pe_tab = uconvert.(u"W", ω2_tab*r2/r1*M1 - m2*g*l*(cos.(ϕ2_tab).*ω2_tab));

# ╔═╡ 7e6b9f1f-4a47-4c06-99b8-0fbfe292c786
begin
	p3 = plot((t_tab, dEkin_tab), label = false, xlabel = "t", ylabel = "dEkin/dt", w = 2, linestyle = :solid, color = :red)
	p4 = plot(t_tab[2:end-1], 100*(dEkin_tab[2:end-1] - Pe_tab[2:end-1])./Pe_tab[2:end-1], label = "rel. Abweichung in %", xlabel = "t")
	pl2 = @layout [a b]
	plot(p3,p4,layout = pl2,size=(600,300),fontfamily="segoe")
end

# ╔═╡ cfd99ee7-3e87-4578-806b-87e42a896530
md"""
Es zeigt sich, dass die zeitliche Änderung der kinetischen Energie praktisch identisch zur Leistung der eingeprägten Kräfte und Kraftmomente ist. Die Bilanz der kinetischen Energie ist erfüllt.

*****

**Auftrag:** Ergänzen Sie den Code so, dass die Leistung auch für beliebige Funktionen ``\chi`` korrekt berechnet wird."""

# ╔═╡ 56d02a32-4327-42fb-af15-f0e4229dcffe
md"""
### Interpolation der kinetischen Energie und Berechnung der Ableitung
"""

# ╔═╡ 294fdedf-9594-4fc1-94bd-4eac3f9b77c4
t_tab

# ╔═╡ e7ecf651-a109-4e8f-947a-43b757e248fc
Ekin_itp = interpolate((ustrip.(u"s", t_tab),), ustrip.(u"J", Ekin_tab), Gridded(Linear()));

# ╔═╡ 592043cb-db7f-4268-bdea-39959818970f
dEkin_fun(t) = only(Interpolations.gradient(Ekin_itp, t))

# ╔═╡ 09fcadee-c11a-4875-8b62-068706800c2e
Ekin_fun(t) = Ekin_itp(t)

# ╔═╡ b98b6b2c-5d25-4656-8d32-fbbeaffc29b1
plot(Ekin_fun,0.0:0.01:sol1.t[end], label = false, xlabel = "t[s]", ylabel = "Ekin [J]", w = 2)

# ╔═╡ 1b4632d3-e3b1-4909-8bd2-f0ce8a866430
plot(dEkin_fun,0.0:0.01:sol1.t[end], label = false, xlabel = "t[s]", ylabel = "dEkin/dt [W]", w = 2)

# ╔═╡ 1c9c6b56-629e-49fc-88a5-db7f8839de6c
dEkin_tab2 = dEkin_fun.(ustrip.(u"s", t_tab))*1u"W";

# ╔═╡ 4648b6c0-d259-4963-86c7-69a03c0e84a7
plot(t_tab[2:end-1], (dEkin_tab[2:end-1] - dEkin_tab2[2:end-1]), label = false)
#./dEkin_tab2[2:end-1]

# ╔═╡ 79a535da-f466-4d5f-b822-6d414b5db8b1
md"""
### Backup"""

# ╔═╡ 545905b1-ed93-44e2-a649-430d67c50c82
begin
	plot(t_tab, dEkin_tab, label = false, ylabel = "dEkin/dt", xlabel = "t")
	plot!(t_tab[1:end-1], aux1, label = false, linestyle = :dash, color = :red)
	plot!(t_tab[2:end], aux1, label = false, linestyle = :dash, color = :magenta)
end
...
manifest_format = "2.0"
project_hash = "d29cddc8be5adc930c5403be8f147a36bff516d3"
