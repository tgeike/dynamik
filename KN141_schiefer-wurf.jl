...
# ╔═╡ 2b582332-be88-464e-9d58-08f286171955
using Unitful,DifferentialEquations,Plots,Printf

# ╔═╡ 07d4aeb4-e591-11ed-1c61-6367fd6a45e4
md"""
# Kinematik und Kinetik mit Julia
## Aufgabe: Schiefer Wurf mit Luftwiderstand
Ein Körper bewegt sich im Schwerefeld der Erde. Die Startgeschwindigkeit und die Starthöhe über dem Erdboden sind bekannt. Auf den Körper wirkt neben der Schwerkraft auch der Luftwiderstand (proportional zum Geschwindigkeitsquadrat). O. B. d. A. erfolgt die Bewegung in der x-y-Ebene (y-Achse entgegengesetzt zur Schwerkraft gerichtet). Es soll nur die translatorische Bewegung untersucht werden. Die Drehbewegung sei (falls überhaupt vorhanden) für die Beantwortung der unten stehenden Fragen nicht relevant. Demnach wird nur die Lage eines Punktes beschrieben.
### Fragen:
1. Welche Höhe erreicht der Körper maximal? Bei welcher x-Position wird die maximale Höhe erreicht und zu welcher Zeit?
2. Wie groß ist die Flugweite? Wie lange dauert der Wurf? Welche Geschwindigkeit hat der Körper unmittelbar vor dem Aufprall (skalarwertige x- und y-Komponente angeben)?"""

# ╔═╡ 704af142-7e4a-472c-9d9f-9f586e911cf1
md"""
!!! task "Auftrag"
    Versuchen Sie, die Bewegungsgleichungen für den schiefen Wurf mit Luftwiderstand eigenständig herzuleiten. Wandeln Sie die Bewegungsgleichungen in ein Dgl.-System 1. Ordnung um. Programmieren Sie die rechte Seite des Dgl.-Systems in einer Funktion `bewegdgl!`.
"""

# ╔═╡ 3596813b-1b2a-47ae-a891-cd786ddee3e1
md"""
### Schritt 1: Parameter und Anfangsbedingungen definieren (mit Einheiten)
Bedenken Sie: Die Parameter können in beliebigen, mit der jeweiligen Größe vereinbaren Einheiten angegeben werden. Die Starthöhe kann z. B. auch in Inch (Zoll) und die Anfangsgeschwindigkeit in Meilen je Stunde angegeben werden. Vor der numerischen Lösung werden die Größen auf zuvor festgelegte Einheiten umgerechnet, und anschließend werden die Einheiten entfernt. In unserem Beispiel werden alle Längen in Metern, alle Geschwindigkeiten in Meter je Sekunde und alle Kräfte in Newton gemessen."""

# ╔═╡ 7b10ffa1-479e-4e3c-b34f-9c2d6bcb0aea
begin
	g = 9.81u"m/s^2"
	m = 58.0u"g"
	k = 1e-3u"N*s^2/m^2"
...
# ╔═╡ 5acac99d-c96f-4ec8-a424-895bf3025e0d
begin
	α = 60*π/180 # Radiant
	y0 = 3.5u"m"
	v0 = 30.0u"m/s"
...
# ╔═╡ 6c5d661d-bdb1-40c5-9202-4749edf6a9fd
md"""Skalarwertige Komponenten der Startgeschwindigkeit"""

# ╔═╡ 3d1388c0-af2e-4008-a006-fd71f882223a
vx0

# ╔═╡ 6a6c79c9-976f-41b2-88b5-f26a4bd4c1ed
vy0

# ╔═╡ dd865d50-9957-405c-99fb-fb790c151040
md"""
### Schritt 2: Bewegungsgleichung definieren
Die Lage des betrachteten Punktes wird über zwei Koordinaten (x, y) beschrieben. Demnach hat der Zustandsvektor ``\underline{z}`` vier Einträge (vier skalare Bestimmungsstücke): ``x``, ``y``, ``v_x = \dot x`` und ``v_y = \dot y``. Zudem sind in der Bewegungsgleichung zwei Parameter: ``k/m`` und ``g``. Beide Parameter werden für die numerische Berechnung ohne Einheiten gespeichert. Auch die Anfangsbedingungen werden ohne Einheiten weiterverarbeitet."""

# ╔═╡ 82180670-3e45-4a13-9a03-0cb2cc38bdac
struct Systemparameter
    C::Float64 # Konstante k/m in 1/Meter
	g::Float64 # Erdbeschleunigung in m/s^2
end

# ╔═╡ b755849e-4b8e-4589-b4b3-5c8a33523394
param = Systemparameter(ustrip(Float64, u"m^-1", k/m), ustrip(Float64, u"m*s^-2", g))

# ╔═╡ 99a000ee-faff-434d-ad97-148af2ae6a99
function bewegdgl!(F,z,p,t) # z = [x,vx,y,vy]
	V = sqrt(z[2]^2 + z[4]^2) # Betrag der Geschwindigkeit
    F[1] = z[2]
    F[2] = -p.C*V*z[2]
	F[3] = z[4]
	F[4] = -p.C*V*z[4] - p.g
...
# ╔═╡ e8be29f4-732f-4a32-acf8-e57ce859ac12
md"""
### Schritt 3: Bewegungsgleichung für die gegebenen Anfangsbedingungen numerisch lösen
Die numerische Integration wird automatisch beendet, wenn der Punkt den Boden erreicht, d. h. wenn die y-Koordinate des Punktes 0 ist."""

# ╔═╡ 531578f2-25bd-47ee-9c3d-e8af640781c4
begin
	tspan = (0.0,6.0)
    z_start = [0.0; ustrip(Float64, u"m*s^-1", vx0); ustrip(Float64, u"m", y0); 
		ustrip(Float64, u"m*s^-1", vy0)]
...
# ╔═╡ 1c28efc0-04b3-4388-aa94-be3c5e47221b
md"""
!!! task "Auftrag"
    Lesen Sie in der Dokumentation nach, der Befehl `ContinuousCallback` funktioniert. Was ist die Bedeutung der Programmzeile `condition(u, t, integrator) = u[3]`?"""

# ╔═╡ b491a471-cc4a-4c25-b2c8-d12a6d38d007
md"""
### Schritt 4: Post Processing
Zuerst wird die Lösung komplett geplottet. Danach werden die Einheiten wieder genutzt und relevante Größen werden in Diagrammen dargestellt.

#### Plotten der Rohdaten
"""

# ╔═╡ e1105a3e-07e0-4593-a9fd-f5d9810de88e
plot(sol1,label=["x [m]" "vx [m/s]" "y [m]" "vy [m/s]"],size=(550,300),xlabel="t [s]")

# ╔═╡ 210f9dc2-e4fd-4af3-8e65-9ad8f14e505c
md"""
#### Berechnen von Wurfweite, Wurfdauer, Scheitelpunktlage, etc."""

# ╔═╡ 0d689167-365f-4bf1-8e5a-5e98eb5e81df
begin
    t_tab = sol1.t*1u"s"
	x_tab = sol1[1,:]*1u"m"
	y_tab = sol1[3,:]*1u"m"
	vx_tab = sol1[2,:]*1u"m/s"
	vy_tab = sol1[4,:]*1u"m/s"
end;

# ╔═╡ cbacf1fe-ed17-434a-8608-5287b1af5536
v_tab = sqrt.(vx_tab.^2 + vy_tab.^2);

# ╔═╡ 094dbb8c-542b-4366-80c9-336911a5c447
y_tab[end] #Prüfe, ob Simulation bei Erreichen des Bodens gestoppt wurde

# ╔═╡ 5d3f3bbc-0a2a-4b46-a451-b96c07604975
x_Wurf = x_tab[end]

# ╔═╡ d83d164c-e903-4b53-9d0d-b37c20f0d78e
t_Wurf = t_tab[end]

# ╔═╡ 356a937f-8bdc-4d9b-9a31-51c015c7d413
v_Aufprall = v_tab[end]

# ╔═╡ d88ecad4-09f3-4d6e-aacb-5d0c20af5f64
y_Scheitel = maximum(sol1[3,:])

# ╔═╡ ced458a1-40d1-438c-9067-cfffe4bbf433
j_Scheitel = argmax(sol1[3,:])

# ╔═╡ c6a6281e-aeac-4b3e-9bd8-ed863aa312f7
x_Scheitel = sol1[1,j_Scheitel]

# ╔═╡ 8a723b7e-9355-4998-83b5-11aa05c36154
t_Scheitel = sol1.t[j_Scheitel]

# ╔═╡ 1e402cb5-27f8-41eb-89ef-ee13a8fae448
md"""**Ergebnisse** 

Der Wurf dauert $(round(ustrip(u"s",t_Wurf),digits=2)) s. Die Wurfweite beträgt $(round(ustrip(u"m",x_Wurf),digits=2)) m. Der Betrag der Aufprallgeschwindigkeit ist $(round(ustrip(u"m/s",v_Aufprall),digits=2)) m/s.

Der Scheitelpunkt hat die Lage ($(round(x_Scheitel,digits=2)) m, $(round(y_Scheitel,digits=2)) m) im gegebenen kartesischen Koordinatensystem. Der Scheitelpunkt wird $(round(t_Scheitel,digits=2)) s nach Start erreicht.
"""

# ╔═╡ 4bdb5920-bdf7-4ddb-9a2d-c4ef357cf380
md"""
#### Weitere Diagramme erstellen"""

# ╔═╡ 0d937248-afdc-4217-80d0-8b9d5d29904c
function circle(x, y, r=1; n=30)
    θ = 0:360÷n:360
    Plots.Shape(r*sind.(θ) .+ x, r*cosd.(θ) .+ y)
end;

# ╔═╡ a96993cd-6c0c-4dee-831a-545afff7d9fc
begin
	plot(x_tab, y_tab, 
		w = 3, 
		label = false, 
		...
		aspect_ratio=:equal,
		framestyle = :origin)
	plot!([x_Scheitel, x_Scheitel], [0, y_Scheitel], label = false, color = :black)
	plot!([0, x_Scheitel], [y_Scheitel, y_Scheitel], label = false, color = :black)
	plot!(circle(x_Scheitel,y_Scheitel,0.5), label = false, color=:white)
end

# ╔═╡ 3d8c06ef-8ef9-4311-aaf2-3c476de6ed43
begin
	plot(t_tab,y_tab,
		w = 3,
		label = false,
		...
		xlabel = "Zeit",
		ylabel = "Höhe")
	plot!([t_Scheitel, t_Scheitel], [0, y_Scheitel], label = false, color = :black)
	plot!([0, t_Scheitel], [y_Scheitel, y_Scheitel], label = false, color = :black)
end

# ╔═╡ 022b3a69-6c46-4245-89b7-1287d1f1a82b
begin
	plot(t_tab, v_tab,
		w = 4,
		label = "Bahngeschw.",
		title = "Geschwindigkeit vs. Zeit",
		xlabel = "Zeit",
		ylabel = "Geschw.")
	plot!([t_Scheitel, t_Scheitel], [vy_tab[end], v_tab[1]], label = false, color = :black)
	plot!(t_tab, vx_tab, label = "x-Richtung")
	plot!(t_tab, vy_tab, label = "y-Richtung")
end

# ╔═╡ 60c11387-c610-483b-b182-eb0023845477
plot(t_tab, 180/π*atan.(vy_tab,vx_tab),
	w = 3, 
	label = false, 
	title = "Tangentenrichtung vs. Zeit", 
	xlabel = "Zeit",
	ylabel = "Winkel gegen Horizontale [°]")
...
manifest_format = "2.0"
project_hash = "4af76d930dddbf811a2ab145d240c80f9d4e33b7"
