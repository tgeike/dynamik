...
# ╔═╡ 533a3013-0420-408c-b4dd-81c97c328313
using DifferentialEquations,Plots,Integrals,PlutoUI

# ╔═╡ ec254312-c8f7-11ef-207f-a9e31ebd91a2
md"""
# Technische Berechnungen mit Julia
## Rimless Wheel -- ein einfaches Modell des Laufens

![Bild](https://github.com/tgeike/dynamik/blob/main/KN446_Rimless-wheel_Bild.png?raw=true)

**Fragestellung:** Wie bewegt sich das randlose Rad (rimless wheel) auf einer schiefen Ebene? Leiten Sie dazu in einem ersten Schritt die Bewegungsgleichung her und lösen sie diese numerisch.

!!! task "Aufgabe"
    Lesen Sie im Skript von Russ Tedrake (MIT) den Abschnitt 4.2.1 durch, siehe Website [Underactuated Robotics](https://underactuated.csail.mit.edu/simple_legs.html#section2).
"""

# ╔═╡ c9ccb34e-c434-4c71-b597-0f8ea8472d4a
md"""
### Laden der Pakete"""

# ╔═╡ a5415a03-a2a8-43d8-8feb-bf52afecc3b8
md"""
### Festlegung der Parameter

| Parameter | Formelzeichen | Wert | Einheit |
|:------------|:-------:|------:|:----:|
| Neigungswinkel | γ | $(@bind γ NumberField(0:0.5:10; default=5.0)) | ° |
| Beinlänge | L | $(@bind L NumberField(0:0.1:10; default=1.0)) | m |
| Zahl der Beine | NB | $(@bind NB NumberField(6:1:12; default=8)) | - |

Die Parameter werden über die Zellen in der vorgegebenen Einheit eingegeben. Es wird im folgenden ohne Einheiten gerechnet.

Die Beinlänge heißt in der Originalaufgabenstellung ``l``. Im Code heißt die Länge der besseren Lesbarkeit wegen `L`. 
...
# ╔═╡ bdd18469-c641-40c8-84a7-a3ea4f1b8650
md"""Der Winkel ``2\alpha`` zwischen zwei Beinen beträgt $(360/NB)° (insgesamt $(NB) Beine)."""

# ╔═╡ 2a846f3c-2cf9-4136-817d-204837a0b28b
g = 9.81 # m/s^2 Erdbeschleunigung

# ╔═╡ d2a9620b-1708-410f-b5b6-10fc5ab04dca
struct Systemparameter
    κ::Float64 # Konstante 1/s - charakteristische Frequenz des Systems
	L::Float64 # Länge der Beine in m
	α::Float64 # Hälfte des Winkels zwischen zwei Beinen in rad
	γ::Float64 # Neigungswinkel der Ebene in rad
end

# ╔═╡ 0ecafa49-98c8-4ef6-82e9-f67a51b2d088
param = Systemparameter(sqrt(g/L),L,deg2rad(360/NB*0.5),deg2rad(γ))

# ╔═╡ 6a6fd070-45a9-4581-9e96-0e68647dfdd4
md"""
### Vorbetrachtung Kinematik

Das Abrollen um den momentanen Aufstandspunkt findet für ``\gamma-\alpha\le\theta\le\gamma+\alpha`` statt.

Es gilt mit den gegebenen Zahlenwerten: $(rad2deg(-param.α+param.γ))° ``\le\theta\le`` $(rad2deg(param.α+param.γ))°.
"""

# ╔═╡ 716cd549-b817-4e3f-b846-f64b81b7fb5d
md"""
### Vorbetrachtung Kinetik
Es gilt die vom stehenden Pendel bekannte Bewegungsgleichung für die Abrollphase (*ein* Kontaktpunkt)
```math
\ddot \theta = \kappa^2 \sin\theta
```
mit
```math
\kappa^2 = \frac{g}{l}\;.
```
!!! task "Aufgabe"
    Leiten Sie die Bewegungsgleichungen eigenständig her. Tipp: Momentengleichung um den Aufstandspunkt verwenden.

Durch Trennung der Veränderlichen kann die Winkelgeschwindigkeit als Funktion des Winkels aus der Bewegungsgleichung bestimmt werden. Die Größe ``\omega_1`` ist die Winkelgeschwindigkeit beim Start aus der Lage mit ``\theta = \gamma-\alpha``.
```math
\omega = \sqrt{\omega_1^2 - 2\kappa^2 \left(\cos\theta - \cos(\gamma-\alpha)\right)}
```
Das gleiche Ergebnis ergibt sich, wenn die Energieerhaltung angesetzt wird.

Während der Kollision eines Fußes mit dem Boden kann als eine denkbare Näherung der Drehimpuls bezüglich des Punktes der Kollision als konstant gesetzt werden. Daraus folgt eine Bedingung für die Winkelgeschwindigkeitsänderung während des Stoßes (siehe unten).
"""

# ╔═╡ b84ed637-7632-423a-8554-b661dad653ba
ω1 = param.κ*sqrt(2*(1-cos(param.γ-param.α)))

# ╔═╡ 3f5d68cb-1fbf-4ad3-bcc0-2e75b2de2420
md"""
Für ein Herunterrollen muss die mechanische Energie reichen, um den Massenmittelpunkt des Systems über den höchsten Punkt (bei ``\theta=0``) zu heben. Dazu muss direkt nach dem Wechsel des Fußes die Winkelgeschwindigkeit des herunterrollenden Rades mindestens 
```math
\omega_1 = \kappa \sqrt{2 (1 - \cos(\gamma + \alpha))}
```
betragen. Dieser **Mindestwert für die Startwinkelgeschwindigkeit** ist für die gegebenen Zahlenwerte **$(round(ω1,digits=3)) rad/s**. Ist die Winkelgeschwindigkeit direkt nach dem Wechsel des Fußes kleiner als dieser Mindestwert, ist ein Herunterrollen des Rades nicht möglich.

!!! task "Aufgabe"
    Leiten Sie das Ergebnis eigenständig her. Tipp: Bewegungsgleichung lösen oder Erhaltung der mechanischen Energie ansetzen.
"""

# ╔═╡ 12c70d24-34f8-46c8-9333-2e8cd5c832ef
md"""
### Numerisches Lösen der Bewegungsgleichung -- ohne Fußwechsel
#### Umsetzen der Bewegungsgleichung und Abbruchbedingungen in Julia Code
"""

# ╔═╡ 6ca12cab-604b-4660-9446-5822da5d7f1c
function bewegdgl!(F,z,p,t) # z = [θ,ω]
	F[1] = z[2]
    F[2] = p.κ^2*sin(z[1])
end;

# ╔═╡ 1b96097a-c640-4740-a0c2-e9634b615663
md"""Die Bewegungsgleichung wird so lange integriert bis der Winkel ``\theta`` entweder den Wert ``\gamma+\alpha`` oder den Wert ``\gamma-\alpha`` erreicht. Das entspricht der Kollision eines Fußes mit dem Boden. Im Moment der Kollision kann nicht mit der Bewegungsgleichung weitergerechnet werden, sondern es müssen zusätzliche Überlegungen zum Drehimpuls herangezogen werden.
"""

# ╔═╡ 91ff6db2-32f5-458f-8bad-f4e71c750533
function abrollphase(zS,p,tS)
	Δtmax = 20.0
	tspan = (tS,tS+Δtmax)
    prob = ODEProblem(bewegdgl!,zS,tspan,p)
	condition(u, t, integrator) = (u[1] - (p.γ+p.α))*(u[1] - (p.γ-p.α))
...
# ╔═╡ b066ab10-b987-4919-a584-a1f29d29d45b
ωRoll = 2*param.κ*sqrt(sin(param.α)*sin(param.γ))/sin(2*param.α)

# ╔═╡ 072cc6b5-4c4e-4090-b5f9-b2b68f92e5e1
md"""
Es zeigt sich, dass bei der Abwärtsbewegung ein Grenzzyklus existiert, bei dem kurz vor dem Aufprall stets die gleiche Winkelgeschwindigkeit vorliegt. D. h. das Rad wird nicht immer schneller, sondern die Abrollphasen wiederholen sich (nahezu) identisch nach Erreichen des stabilen Grenzzyklus. Für die vorliegenden Werte beträgt die **Winkelgeschwindigkeit kurz vor dem Aufprall für den Grenzzyklus $(round(ωRoll,digits=3)) 1/s**.
"""

# ╔═╡ f63c2c5d-6417-4abd-89aa-a10730169eb0
md""" 
#### Numerisches Lösen des AWP

Startorientierung: ``\theta_\mathrm{S}=\theta(t=0) = \gamma - \alpha``

Startwinkelgeschwindigkeit ``\omega_\mathrm{S}``: $(@bind ωS NumberField(0.5:0.1:2; default=1.0)) 1/s"""

# ╔═╡ fa38b628-410e-4b77-89d3-f7f794e6613b
md"""Wir starten aus der Orientierung ``\theta = \gamma - \alpha`` mit Winkelgeschwindigkeit ``\omega_\mathrm{S}``=$(round(ωS,digits=3)) rad/s. Zur Erinnerung: der Mindestwert für die Winkelgeschwindigkeit für die gegebenen Zahlenwerte beträgt $(round(ω1,digits=3)) rad/s, d. h. der Startwert ist $(round(ωS/ω1,digits=3)) ``\omega_1``."""

# ╔═╡ 2350ced1-ed85-4098-9f72-29667d8809a8
sol1 = abrollphase([param.γ-param.α ωS],param,0.0);

# ╔═╡ c486acb5-5278-4950-891d-cc212936108f
plot(sol1,label=["θ" "ω"],lw=2,size=(400,250),xlabel="t [s]")

# ╔═╡ beb8a59b-c4a0-4b75-bff3-0c512d29fadd
md"""Betrachten Sie das oben stehende Diagramm. Wenn der Winkel ``\theta`` monoton wächst, rollt das Rad während der Abrollphase ausschließlich herunter. Andernfalls rollt das Rad anfangs herunter und im zweiten Abschnitt der Abrollphase wieder zurück (nach oben)."""

# ╔═╡ 2ee83d9a-a053-402b-ac10-6503fe87bdde
md"""
!!! task "Aufgabe"
    Probieren Sie aus was passiert, wenn Sie die Anfangswinkelgeschwindigkeit ``\omega_\mathrm{S}`` verändern.
"""

# ╔═╡ 60f808ad-5d5e-40b3-a940-d7d31e163774
md"""
Aus theoretischen Überlegungen ist die Winkelgeschwindigkeit unmittelbar vor der Kollision des nächsten Fußes (beim Herunterrollen) durch die Formel
```math
\omega_\mathrm{vK} = \sqrt{\omega_\mathrm{S}^2 + 4 \kappa^2 \sin\gamma\sin\alpha}
```
gegeben. Das kann mit dem numerisch berechneten Wert verglichen werden (sofern das Rad herunterrollt und nicht zurückpendelt).
"""

# ╔═╡ dfbc6350-48a8-4d87-9bda-21508f64cc50
ωvK = sqrt(ωS^2 + 4*param.κ^2*sin(param.α)*sin(param.γ))

# ╔═╡ 1242b295-8181-4e68-badf-ce59699be232
md"""Abweichung: $(ωvK - sol1[2,length(sol1.t)])"""

# ╔═╡ c2dd064e-1280-493a-b55c-fb2a1529bf80
md"""
(Ungefähr) simulierte Gesamtzeit $(@bind tmax NumberField(1.0:1:10.0; default=8.0)) s"""

# ╔═╡ 3d770019-9ef7-433d-8ef0-581463dfe73e
md"""
### Numerisches Lösen der Bewegungsgleichung mit Fußwechsel

Nun wird die Bewegung des Systems einschließlich der Kollision des Fußes mit dem Boden gerechnet. Dazu wird im betrachteten Zeitintervall, hier [0 s, $(tmax) s], zuerst das Abrollen wie oben und schließlich die Kollision und dann wieder das Abrollen u.s.w. berechnet. Die Kollision wird als Prozess, der in unendlich kurzer Zeitspanne geschieht, angesehen."""

# ╔═╡ ba0a131c-06f0-42b0-a2ce-16b347f47de2
begin
	T_thres = 0.4 # s
	T_tab = Any[]
	t_tab = copy(sol1.t)
	...
	ω_tab = copy(sol1[2,:])
	ωvK_tab = [ω_tab[end]]
	while t_tab[end] < tmax
		push!(T_tab,t_tab[end])
		θvK = θ_tab[end] 
		ωnK = cos(2*param.α)*ω_tab[end] 
		tK = t_tab[end] 
		if abs(θvK - (param.α+param.γ)) < 1e-3 
			sol2 = abrollphase([param.γ-param.α,ωnK],param,tK) 
		elseif abs(θvK - (-param.α+param.γ)) < 1e-3 
			sol2 = abrollphase([param.γ+param.α,ωnK],param,tK)
		end 
		push!(t_tab,copy(sol2.t)...) 
		push!(θ_tab,copy(sol2[1,:])...) 
		push!(ω_tab,copy(sol2[2,:])...) 
		push!(ωvK_tab,ω_tab[end])
		if length(T_tab) > 1 && (T_tab[end] - T_tab[end-1]) < T_thres
			break
		end
	end
end;

# ╔═╡ bb312c99-0252-4cd5-bc5c-4e6b55a6c4d0
md"""
### Post Processing
##### Winkel und Winkelgeschwindigkeit als Funktion der Zeit aufgetragen"""

# ╔═╡ 87dd7183-3841-4d98-b5ab-6b233218cacc
plot(t_tab,[θ_tab ω_tab],label=["θ [rad]" "ω [1/s]"],lw=[1 2])

# ╔═╡ a937d01c-a673-4571-ad8f-e1dfa203e6a9
md"""
##### Winkelgeschwindigkeit über dem Winkel aufgetragen (Phasenraumdarstellung)
Diese Darstellung hat den Vorteil, dass man die Annäherung der Bewegung an den Grenzzyklus gut erkennen kann.
"""

# ╔═╡ 0f34af90-02f3-4595-a318-878a3c7ec46c
begin 
	plot(θ_tab,ω_tab,label=false,ylabel="ω [1/s]",xlabel="θ [rad]",lw=2)
	vline!([-param.α+param.γ param.α+param.γ],label=false,line=:dash,lw=2)
	hline!([ωRoll,ω1],label=false,line=:dash,lw=2)
end

# ╔═╡ 30435f78-2bdb-4fa2-9537-9d2b14877761
md"""Wenn die Startwinkelgeschwindigkeit ausreichend groß ist, rollt das Rad die Ebene herunter. Es stellt sich ein Grenzzyklus ein."""

# ╔═╡ 7617db7e-ad34-4b8f-abfa-46ff5906d0f0
diff(T_tab)

# ╔═╡ dacb221c-ee2f-4ed1-8b89-e8c5867f4def
begin 
	scatter(ωvK_tab,label=false,title="Winkelgeschwindigkeit vor der Kollision", size=(470,250))
	hline!([ωRoll],label=false,line=:dash,lw=2)
end

# ╔═╡ 9f7a524e-903e-4c17-b590-1126f65378ec
md"""
#### Dauer zwischen zwei Kollisionen und durchschnittliche Winkelgeschwindigkeit
Die Zeit kann mithilfe elliptischer Integrale auch analytisch berechnet werden.
"""

# ╔═╡ b8aadd4e-6643-47eb-a24e-417cdfd4623f
scatter(diff(T_tab),label=false,title="Zeit zw. 2 Kollisionen [s]",size=(470,250))

# ╔═╡ e32047ed-6e7f-422c-8f57-14d94f847b9c
md"""Durchschnittliche Winkelgeschwindigkeit"""

# ╔═╡ 9fb3f413-56e2-4930-b5d6-212b48fe37e7
2*param.α./diff(T_tab)

# ╔═╡ fc0431c2-b9e9-4f7e-91f8-b9f1f5c18a04
md"""
### Backup
"""

# ╔═╡ fe207115-3ba9-418d-8530-a25cb7efec8d
md"""Wir berechnen die notwendige Zeit für die erste Abrollphase durch numerische Integration.
```math
T = \frac{1}{\sqrt{2}\kappa}\int_{\gamma-\alpha}^{\gamma+\alpha}\frac{d\theta}{\sqrt{a-\cos\theta}}
```
mit
```math
a = \frac{1}{2}\left(\frac{\omega_1}{\kappa}\right)^2 + \cos(\gamma-\alpha)
```
"""

# ╔═╡ 2f86a7de-9cd8-4987-bc11-f26f412a4306
a(ω1) = 0.5*(ω1/param.κ)^2 + cos(param.γ-param.α);

# ╔═╡ d4084df4-a1c7-4c47-ab0f-ba714ca0e388
f(ξ,ω1) = 1/(sqrt(2)*param.κ*sqrt(a(ω1)-cos(ξ)));

# ╔═╡ 3e50f810-7d60-4259-a0b9-ce63e5c6d809
I1 = solve(IntegralProblem((ξ,p) -> f(ξ,ωS), (param.γ-param.α,param.γ+param.α)), QuadGKJL())

# ╔═╡ de6c848e-9b47-4f69-8430-74d0a0946bc7
T_tab[1] - I1.u

# ╔═╡ a555cbac-ba8d-48b7-b4ea-1ac6e8559057
IRoll = solve(IntegralProblem((ξ,p) -> f(ξ,ωRoll*cos(2*param.α)), (param.γ-param.α,param.γ+param.α)), QuadGKJL())

# ╔═╡ 336a03e4-375d-4a69-9033-c052fbe6a249
begin 
	scatter(diff(T_tab),label=false,title="Zeit zw. 2 Kollisionen [s]",size=(470,250))
	hline!([IRoll.u],label=false,line=:dash,lw=2)
end
...
manifest_format = "2.0"
project_hash = "b153080ef76e79768ad66e2a8a1fc658260c5a16"
