...
# ╔═╡ 661318f6-25bb-4476-9651-5210eb796bc5
using PlutoUI

# ╔═╡ 3f05b6d9-f5e2-4c86-87fb-639d44040ff6
using Plots, ForwardDiff, DifferentialEquations

# ╔═╡ 163d0950-c497-11f0-3b3e-edf601c29882
md"""
# Kinematik und Kinetik mit Julia
## Zwei Körper mit Seil
Gesucht ist die Bewegung des abgebildeten Systems, wenn Körper 1 zum Startzeitpunkt um die Länge ``\delta`` nach unten aus der statischen Ruhelage ausgelenkt wird. Beide Körper bewegen sich ausschließlich translatorisch in vertikaler Richtung. Reibung zwischen Körper 1 und der Führungsschiene sowie Trägheit und Dehnbarkeit des Seils werden vernachlässigt. Die Trägheit und die Reibung in der Umlenkrolle werden ebenfalls vernachlässigt.

Diese Aufgabe ist im Abschnitt 10.3 des Lehrbuchs *Kinematik und Kinetik* von T. Geike enthalten.
"""

# ╔═╡ 0c8afadc-45e9-486e-bb45-75727c0b8ad7
md"""
![System](https://github.com/tgeike/dynamik/blob/main/KN453_Seilzug-nichtlinear.png?raw=true)
"""

# ╔═╡ 74d53b68-5e0b-47ac-bf0f-72a070cf5221
md"""
!!! task "Aufgabe"
    Leiten Sie die Bewegungsgleichung eigenständig her. Beginnen Sie damit, dass Sie die kinematische Beziehung zwischen den beiden Koordinaten ``y_1`` und ``y_2`` herleiten.
"""

# ╔═╡ 4e6976bb-0e02-425d-8123-ea31b46a15c8
md"""Das Paket `PlutoUI` wird für die Eingabezellen benötigt. Das Paket `Plots` dient der Diagrammerstellung, `ForwardDiff` der automatischen Differentiation und `DifferentialEquations` dem Lösen des Anfangswertsproblems (AWP)."""
...
|:------------|:-------:|------:|:----:|
| Horizontaler Abstand der Körper | d | $(@bind d NumberField(0:0.1:10; default=2.0)) | m |
| Masse Körper 1 | m1 | $(@bind m1 NumberField(0:0.5:10; default=2.0)) | kg |
| Masse Körper 2 | m2 | $(@bind m2 NumberField(0:0.5:10; default=4.0)) | kg |
| Seillänge | L | $(@bind L NumberField(0:0.5:20; default=10.0)) | m |
| Anfangsauslenkung aus der stat. Ruhelage | δ | $(@bind δ NumberField(0:0.05:1; default=0.1)) | m |

Die Parameter werden über die Zellen in der vorgegebenen Einheit eingegeben. Es wird im folgenden ohne Einheiten gerechnet. Da die Zeit in Sekunden gezählt wird, sind alle berechneten Geschwindigkeiten in m/s und alle Beschleunigungen in m/s².

Die Seillänge heißt in der Originalaufgabenstellung ``l``. Im Code heißt die Länge der besseren Lesbarkeit wegen `L`.
...
Vorbemerkung: Da sich beide Körper nur translatorisch bewegen, ist es unerheblich welchen Punkt des jeweiligen Körpers wir mit den Koordinaten ``y_1`` bzw. ``y_2`` vermaßen. 

Für die konstante Länge des Seiles ``l`` ergibt sich aus der Geometrie der Anordnung bei straffem Seil
```math
	l = \sqrt{y_1^2 + d^2} + y_2 \; .
```
Aufgelöst nach $y_2$ ergibt sich die kinematische Beziehung in der Form
```math
	y_2 = l - \sqrt{y_1^2 + d^2}\; .
```
Durch Differenzieren nach der Zeit ergibt sich die Geschwindigkeit von Körper 2
```math
	v_2 = \dot y_2 = -\frac{y_1}{\sqrt{y_1^2 + d^2}} \, \dot y_1 \; .
```
...
md"""
Allgemein kann man für die kinematische Beziehung auch schreiben
```math
	y_2 = \psi(y_1) \text{ mit } \psi(y_1) = l - \sqrt{y_1^2 + d^2}\; .
```
Für die Geschwindigkeit ``v_2=\dot y_2`` gilt dann formal
```math
	v_2 = \dot y_2 = \frac{\partial \psi}{\partial y_1}\dot y_1 = -\frac{y_1}{\sqrt{y_1^2 + d^2}} \, \dot y_1 \; .
```
"""
...
md"""
!!! task "Aufgabe"
    Leiten Sie eigenständig die Formel für die Beschleunigung her. Zeigen Sie, dass in der Formulierung über ``\psi`` die Beschleunigung von Körper 2 sich schreiben lässt als 
    ```math 
    a_2 = \ddot y_2 = \partial_1\psi\cdot\ddot y_1 + \partial_{11}\psi\cdot \dot y_1^2\;.
    ```
    Dabei wurde folgende abkürzende Schreibweise gewählt.
    ```math
    \partial_1\psi := \frac{\partial \psi}{\partial y_1} \quad\text{und}\quad \partial_{11}\psi := \frac{\partial^2 \psi}{\partial y_1^2}
    ```
"""

# ╔═╡ 6820869c-d17c-4d98-b106-f54935a75882
ψ(y1) = L - sqrt(y1^2 + d^2);

# ╔═╡ 70efffc7-ed0a-409e-9df4-866b10c033ea
∂1ψ(y1) = ForwardDiff.derivative(ψ,y1);

# ╔═╡ f7b4ef90-6819-4fae-8dd7-376f00a81a44
∂11ψ(y1) = ForwardDiff.derivative(∂1ψ,y1);
...
Der Ausdruck für den Zwang ``\mathcal{Z}`` ist hier besonders einfach, weil die Bewegung ausschließlich in ``y``-Richtung erfolgt und je Körper nur eine eingeprägte Kraft (die Gewichtskraft) wirkt. 
```math
	\mathcal{Z} = \frac{1}{2m_1}\left(m_1 a_1 - m_1 g\right)^2 + \frac{1}{2m_2}\left(m_2 a_2 - m_2 g\right)^2
```
Das Prinzip des kleinsten Zwangs lautet mit ``a_1 = \ddot y_1`` und ``a_2 = \partial_1\psi\cdot\ddot y_1 + \partial_{11}\psi\cdot \dot y_1^2``
```math
	\mathcal{Z}(\ddot y_1) = \mathrm{min!}\;.
```
"""
...
manifest_format = "2.0"
project_hash = "08aaf0a82f369a4b0554c38ce6f7ecb5aa9d75a1"
...
git-tree-sha1 = "e0a34dfe45f6801e2a972fc3866094eb04791c90"
