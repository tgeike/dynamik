...
param = Systemparameter(ustrip(Float64, u"s^(-1)", sqrt(k*l^2/JA)), ustrip(Float64, NoUnits, m*g/(k*l)))

# ╔═╡ ff350a69-f1f5-4f0c-b343-8bce2ada1f59
md"""
### Potentielle Energie untersuchen
Gleichgewichtslagen zeichnen sich dadurch aus, dass die potentielle Energie dort lokale Extrema aufweist. Lokale Minima weisen auf stabile Gleichgewichtslagen hin.

Wir betrachten die dimensionslose potentielle Energie
```math
\bar E_\mathrm{pot} = \frac{2 E_\mathrm{pot}}{k l^2} = (\kappa-4)\cos\varphi - 2\sqrt{5-4\cos\varphi} + 6\;.
```
Die potentielle Energie ``E_\mathrm{pot}`` des Systems ist genau dann minimal, wenn die dimensionslose potentielle Energie ``\bar E_\mathrm{pot}`` minimal ist.
"""

# ╔═╡ 74919456-7847-4fdd-acc4-c4c2e96a727b
md"""Die stabile Gleichgewichtslage kann analytisch oder numerisch aus der potentiellen Energie bestimmt werden. Hier wird beides gemacht. `ϕG` ist eine Funktion, die in Abhängigkeit vom Parameter `κ` den Winkel der Gleichgewichtslage bestimmt. Weiter unten wird mit dem Befehl `optimize` aus dem Paket `Optim` der Winkel der Gleichgewichtslage numerisch berechnet."""

# ╔═╡ 32acf76d-b9fa-4e2b-8568-5f70c3ccdefb
ϕG(κ) = acos(0.25*(5-(4/(4-κ))^2));

# ╔═╡ 6de29f58-6815-4120-9ac5-a40d4f8d6b74
ϕG(param.κ)
...
# ╔═╡ e2d48c31-fa9f-4eac-80ad-dde103023420
begin 
	plot(0.0:0.02:π,ϕ->Epot_d(ϕ,param.κ),label="pot. Energie",lw=3)
	vline!([ϕG(param.κ)],label="Minimalstelle")
end

# ╔═╡ c11cfa4e-9607-47c9-a972-9ec38c03e08d
...
md"""
### Bewegungsgleichung numerisch lösen
Jetzt sollen Schwingungen um die stabile Gleichgewichtslage untersucht werden.
"""
...
```math
\ddot\varphi = \Xi(\varphi) = -\lambda^2\left[2\left(1-\frac{1}{\sqrt{5-4\cos\varphi}}\right)-\frac{\kappa}{2}\right]\sin\varphi\;.
```
...
# ╔═╡ 1b9e2977-1e4a-4af5-94d2-f00aea82c520
ϕ_tab = sol[1,:];

# ╔═╡ 6b47688d-4fe5-4eb7-8da9-e59ab2176049
begin 
	plot(sol.t,ϕ_tab,label=false,size=(500,300),lw=3,xlabel="Zeit [s]",ylabel="Auslenkung [m]")
	vline!([T 2*T 3*T],label=false)
end
...
Emech_d_tab = Epot_d.(ϕ_tab,param.κ) + Ekin_d.(ω_tab,param.λ);
...
manifest_format = "2.0"
project_hash = "1cc273fd86ea3dffa543e4f2d0a14adb13065641"
...
git-tree-sha1 = "b67fb758e019d833d6627a61cfdaff052ef46361"
